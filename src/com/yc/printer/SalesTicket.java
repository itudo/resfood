...
public class SalesTicket implements Printable {
	...
	private String companyName = "衡阳源辰信息技术有限公司";
	 ...
	 * 小票纸宽度一般为58mm，大概为165点 pageIndex - 要绘制的页面从 0 开始的索引 ，即页号
	 */
	@Override
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		// 打印起点坐标
		double x = pageFormat.getImageableX(); // 返回与此 PageFormat相关的
												// Paper对象的可成像区域左上方点的 x坐标。
		double y = pageFormat.getImageableY(); // 返回与此 PageFormat相关的
												// Paper对象的可成像区域左上方点的 y坐标。
		...
		if (goods != null && goods.size() > 0) {
			for (Resorderitem gdf : goods) {
				...
				g2.drawString(String.valueOf(gdf.getDealprice()), (float) x + 60, (float) y + line);
